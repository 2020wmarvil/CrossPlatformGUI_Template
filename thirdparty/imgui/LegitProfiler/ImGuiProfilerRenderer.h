...
public:
    ...
    void RenderTimings(int graphWidth, int legendWidth, int height, int frameIndexOffset)
    {
        ...
        RenderGraph(drawList, widgetPos, ImVec2(graphWidth, height), frameIndexOffset);
        RenderLegend(drawList, widgetPos + ImVec2(graphWidth, 0.0f), ImVec2(legendWidth, height), frameIndexOffset);
...
private:
    ...
    void RenderGraph(ImDrawList *drawList, ImVec2 graphPos, ImVec2 graphSize, size_t frameIndexOffset)
    {
        ...
        for (size_t frameNumber = 0; frameNumber < frames.size(); frameNumber++)
        {
            ...
            ImVec2 framePos
                = graphPos
                  + ImVec2(graphSize.x - 1 - frameWidth - (frameWidth + frameSpacing) * frameNumber, graphSize.y - 1);

            if (framePos.x < graphPos.x + 1)
                break;
            ImVec2 taskPos = framePos + ImVec2(0.0f, 0.0f);
            auto &frame    = frames[frameIndex];
            for (const auto &task : frame.tasks)
            {
                ...
                if (abs(taskEndHeight - taskStartHeight) > heightThreshold)
                    Rect(drawList,
                         taskPos + ImVec2(0.0f, -taskStartHeight),
                         taskPos + ImVec2(frameWidth, -taskEndHeight),
                         task.color,
                         true);
            }
        }
    }
    void RenderLegend(ImDrawList *drawList, ImVec2 legendPos, ImVec2 legendSize, size_t frameIndexOffset)
    {
        ...
        for (auto &taskStat : taskStats) { taskStat.onScreenIndex = size_t(-1); }
        ...
        for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++)
        {
            ...
            ImVec2 markerLeftRectMin = legendPos + ImVec2(markerLeftRectMargin, legendSize.y);
            ImVec2 markerLeftRectMax = markerLeftRectMin + ImVec2(markerLeftRectWidth, 0.0f);
            markerLeftRectMin.y -= taskStartHeight;
            markerLeftRectMax.y -= taskEndHeight;

            ImVec2 markerRightRectMin
                = legendPos
                  + ImVec2(markerLeftRectMargin + markerLeftRectWidth + markerMidWidth,
                           legendSize.y - markerRigthRectMargin
                               - (markerRightRectHeight + markerRightRectSpacing) * stat.onScreenIndex);
            ImVec2 markerRightRectMax = markerRightRectMin + ImVec2(markerRightRectWidth, -markerRightRectHeight);
            ...
            Text(drawList,
                 markerRightRectMax + textMargin + ImVec2(nameOffset, 0.0f),
                 textColor,
        ...
        for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++)
        {
          ...
          if (it != priorityEntries.end() && !it->second.isUsed)
          {
            ...
            ImVec2 markerRightRectMin = legendPos + ImVec2(markerLeftRectMargin + markerLeftRectWidth + markerMidWidth,
        legendSize.y - markerRigthRectMargin - (markerRightRectHeight + markerRightRectSpacing) * shownTasksCount);
            ImVec2 markerRightRectMax = markerRightRectMin + ImVec2(markerRightRectWidth, -markerRightRectHeight);
        ...
        task.color);
            ...
            Text(drawList, markerRightRectMax + textMargin, textColor, timeText.str().c_str());
            Text(drawList, markerRightRectMax + textMargin + ImVec2(nameOffset, 0.0f), textColor, (std::string("ms] ") +
        ...
        /*for (size_t priorityTaskIndex = 0; priorityTaskIndex < priorityTasks.size(); priorityTaskIndex++)
        {
          ...
          ImVec2 taskPos = legendPos + marginSpacing + ImVec2(0.0f, markerHeight) + ImVec2(0.0f, (markerHeight +
        itemSpacing) * priorityTaskIndex); Rect(drawList, taskPos, taskPos + ImVec2(markerHeight, -markerHeight),
        task.color, true); Text(drawList, taskPos + textOffset, 0xffffffff, task.name.c_str());
        }*/
    }
    ...
    static void Triangle(ImDrawList *drawList, std::array<ImVec2, 3> points, uint32_t col, bool filled = true)
    {
        if (filled)
    ...
    static void RenderTaskMarker(ImDrawList *drawList,
                                 ImVec2 leftMinPoint,
                                 ImVec2 leftMaxPoint,
                                 ImVec2 rightMinPoint,
                                 ImVec2 rightMaxPoint,
                                 uint32_t col)
    {
        ...
        /*void BuildPriorityTasks(size_t maxPriorityTasksCount)
        {
          ...
          for (size_t priorityIndex = 0; priorityIndex < maxPriorityTasksCount; priorityIndex++)
          {
            ...
            for (size_t taskIndex = 0; taskIndex < tasks.size(); taskIndex++)
            {
              ...
              if (it == usedTaskNames.end() && (bestTaskIndex == size_t(-1) || tasks[bestTaskIndex].GetLength() <
        task.GetLength()))
              {
